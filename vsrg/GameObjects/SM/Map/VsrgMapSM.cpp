...
void VsrgMapSM::processBpms(const std::vector<std::string>& str_v) {
	...
	*/
	...
	int measure = 0, measure_prev = 0;
	double bpm = 0.0, bpm_prev = 0.0;
	TimingGridMeasure tgm = TimingGridMeasure();
	for (const auto& str : str_v) {
		size_t sep_loc = str.find('=');
		measure = std::stoi(str.substr(str.find_first_of(',') == std::string::npos ? 0 : 1, sep_loc));
		bpm = std::stod(str.substr(sep_loc + 1));
		for (int m = 0; m < (measure - measure_prev); m++) {
			tgm = TimingGridMeasure(4, 192, bpm);
		}
		tgb.pushMeasure(tgm);
