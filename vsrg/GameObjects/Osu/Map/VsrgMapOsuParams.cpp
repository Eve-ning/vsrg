...
VsrgMapOsuParams::VsrgMapOsuParams(
	...
	bg_path_(bg_path), source_(source), tags_(tags),
...
VsrgMapOsuParams::VsrgMapOsuParams(
	double hp, double od, unsigned int keys, unsigned int preview_time,
	const std::string & bg_path,
	const std::string & source,
	const std::string & title,
	const std::string & artist,
	...
	hp_(hp), od_(od), keys_(keys), bg_path_(bg_path), source_(source),
	beatmap_id_(0), beatmap_set_id_(-1), preview_time_(preview_time) {}
